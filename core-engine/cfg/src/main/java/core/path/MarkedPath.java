    ...
    private static CfgNode findUncoveredStatement(CfgNode rootNode, CfgNode duplicateNode) {
        ...
        if (!coveredNodeInPath.contains(rootNode)) {
            ...
            if (rootNode instanceof CfgBoolExprNode) {
                ...
                CfgNode noMoreElementUncoveredNode = findUncoveredStatement(forEachNode.getNoMoreElementAfterNode(), duplicateNode);
                CfgNode hasElementUncoveredNode = findUncoveredStatement(forEachNode.getHasElementAfterNode(), duplicateNode);
                return noMoreElementUncoveredNode == null ? hasElementUncoveredNode : noMoreElementUncoveredNode;
            } else {
                if (rootNode != ConcolicTestingWithStub4Libs.previousUncoveredNode && !rootNode.isMarked() && rootNode.isFakeMarked() < 5
                        && !rootNode.getContent().isEmpty()) {
                    return rootNode;
                }
...
>>>>>>> 4719efc0cc44b3e122543b87d7578eb9575b2a7a
                ...
                if (forEachNode != ConcolicTestingWithStub4Libs.previousUncoveredNode &&
                        !forEachNode.isHasElementMarked() && forEachNode.getFakeHasElementMarked() < 5) {
                    isGoingTrueBranch = true;
                    return forEachNode;
                }
                if (forEachNode != ConcolicTestingWithStub4Libs.previousUncoveredNode &&
                        !forEachNode.isNoMoreElementMarked() && forEachNode.getFakeNoMoreElementMarked() < 5) {
                    isGoingTrueBranch = false;
                    return forEachNode;
                }

                CfgNode noMoreElementUncoveredNode = findUncoveredBranch(forEachNode.getNoMoreElementAfterNode(), duplicateNode);
                CfgNode hasElementUncoveredNode = findUncoveredBranch(forEachNode.getHasElementAfterNode(), duplicateNode);
                return noMoreElementUncoveredNode == null ? hasElementUncoveredNode : noMoreElementUncoveredNode;
            } else {
