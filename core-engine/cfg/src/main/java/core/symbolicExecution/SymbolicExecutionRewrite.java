...
public class SymbolicExecutionRewrite {
    ...
    public List<Z3VariableWrapper> execute() {
            ...
            for (ASTNode param : this.parameters) {
                if (param instanceof SingleVariableDeclaration) {
                    ...
                    if (decl.getType().toString().equals("char")) {
                        ...
                        BoolExpr constraint = ctx.mkLe(ctx.mkLength(strVar), ctx.mkInt(maxLength));

                        // Mọi ký tự <= 0xFFFF
                        for (int i = 0; i < maxLength; i++) {

                            BoolExpr positionExists =
                                    ctx.mkLt(ctx.mkInt(i), ctx.mkLength(strVar));

                            BoolExpr charConstraint = ctx.mkLe(
                                    ctx.charToInt(ctx.mkNth(strVar, ctx.mkInt(i))),
                                    ctx.mkInt(0xFFFF)
                            );

                            constraint = ctx.mkAnd(
                                    constraint,
                                    ctx.mkImplies(positionExists, charConstraint)
                            );
                        }
        ...
        while (currentNode != null) {
            ...
            // currentCfgNode.getContent().isEmpty() ? currentCfgNode.getParent() + "go!" : currentCfgNode
            if (ngu == 39) {
                System.out.println("koko");
            }
            System.out.println(++ngu + " { } " + (currentCfgNode));
